...
class IOConnection implements IOCallback {
	...
	/** true if there's already a keepalive in {@link}. */
	...
	private void handshake() {
		...
		try {
			...
			System.out.println(post.getURI().toString() + "Socket:" + SOCKET_IO_1);
	...
	public HttpClient getNewHttpClient() {
		try {
			...
			WebClientSSL sf = new WebClientSSL(trustStore);
	...
	public void transportMessage(String text) {
		...
		case IOMessage.TYPE_EVENT:
			try {
				...
				try {
					IOCallback ioCallback = findCallback(message);
					ioCallback.on(eventName, remoteAcknowledge(message), argsArray);
				} catch (Exception e) {
					e.printStackTrace();
	...
	public void on(String event, IOAcknowledge ack, Object... args) {
		for (SocketIO socket : sockets.values()) {
			IOCallback ioCallback = socket.getCallback();
